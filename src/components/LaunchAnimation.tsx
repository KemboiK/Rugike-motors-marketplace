...
const LaunchAnimation = ({ onAnimationComplete }: LaunchAnimationProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-Ndai-primary overflow-hidden">
      {/* Cinematic elements */}
      <div className={`absolute inset-0 ${animationStage !== 'intro' ? 'opacity-100' : 'opacity-0'} transition-opacity duration-1000`}>
        <div className="absolute inset-0 bg-gradient-to-br from-Ndai-dark via-Ndai-primary to-black"></div>
      ...
      <div className={`absolute w-full h-full overflow-hidden ${animationStage === 'logo-reveal' || animationStage === 'sponsor' ? 'opacity-40' : 'opacity-0'} transition-opacity duration-1500`}>
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[200%] h-40 bg-Ndai-accent/30 rotate-45 blur-3xl"></div>
      ...
      <div className="text-center relative z-10">
        {/* Ndai Motors Logo Animation - Now with cinematic reveal */}
        <div className={`transform ${animationStage === 'logo-reveal' || animationStage === 'sponsor' ? 'scale-100 opacity-100' : 'scale-90 opacity-0'} transition-all duration-1000 ease-out`}>
          ...
          <div className={`h-1 w-0 bg-Ndai-accent mx-auto my-3 transition-all duration-3000 ${animationStage === 'logo-reveal' || animationStage === 'sponsor' ? 'w-48 md:w-80' : 'w-0'}`}></div>
          <p className={`text-Ndai-light text-xl opacity-0 transition-opacity duration-1000 ${animationStage === 'logo-reveal' || animationStage === 'sponsor' ? 'opacity-80 delay-500' : ''}`}>
        ...
        <div className={`mt-12 transform transition-all duration-1000 ${animationStage === 'sponsor' ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          <p className="text-Ndai-secondary text-sm mb-2">
